...
ts_act_power_hourly <- msts(act_power_hourly$hourly_mean_active_power, 
                            seasonal.periods =c(24,168,8766), #based on hourly periods as opposed to daily periods
                            start=c(2006,12,16))

ts_act_power_daily <- msts(act_power_daily$daily_mean_active_power, 
                           seasonal.periods =c(7,365.25), #accounts for leap years
                           start=c(2006,12,16))
...
### Model 1: STL + ETS (Seasonal Trend Linear + Error Trend Seasonal model - combining non-seasonal model with seasonal model)
...
### Model 2: ARIMA + FOURIER terms (ARIMA to capture trend and Fourier terms to represent seasonality - Fourier represents multiple types of seasonality)
...
### Model 3: TBATS - like ES but can do multiple aspects
