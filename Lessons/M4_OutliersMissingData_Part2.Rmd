...
wind_decomp_time<-cbind(full_wind_data[,1], wind_random, wind_seasonal, wind_trend)

#Ex4: Inspect the data frame using head() and tail()

head(wind_decomp_time)
tail(wind_decomp_time)

# Discuss the results

plot(wind_decomp_time) #Can't see the trend because it's calculated with a centered moving average - whereas seasonal is calculated with the seasonal means model - which assumes that the month-level means are the same across all seasons
