...
linear_trend <- lm(inflow_data[,iHP+1] ~ t) #just need some increasing vector for the x - doesnt need to be dates
...
dummies <- seasonaldummy(ts_inflow_data[,iHP]) #creates dummy variables that gives a 1 when the observation has the given month. requires period created in time series. Note that it will leave one out to permit an intercept

#regression with seasonal dummy

seas_means_model<-lm(inflow_data[,(iHP+1)]~dummies)
summary(seas_means_model) #all of these values will be relative to the intercept - in this case the december mean

beta0<-seas_means_model$coefficients[1]
beta1<-seas_means_model$coefficients[2:12]

inflow_seas_comp <- array(0,nobs)#just creating array before filling it in - instead of appending a smaller array
for(i in 1:nobs){
